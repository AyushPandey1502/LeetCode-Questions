...
public:
    string nearestPalindromic(string n) {
        ...
        for (long long i : {-1, 0, 1}) {
            ...
            if (length % 2 == 0) {
                candidate =
                    newPrefix + string(newPrefix.rbegin(), newPrefix.rend());
            } else {
                candidate = newPrefix +
                            string(newPrefix.rbegin() + 1, newPrefix.rend());
            }
            candidates.insert(candidate);
        }

        candidates.insert(
            to_string(static_cast<long long>(pow(10, length - 1)) - 1));
        candidates.insert(
            to_string(static_cast<long long>(pow(10, length)) + 1));
        ...
        for (const string& candidate : candidates) {
            ...
            if (difference < minDifference ||
                (difference == minDifference &&
                 candidateNum < stoll(closestPalindrome))) {
